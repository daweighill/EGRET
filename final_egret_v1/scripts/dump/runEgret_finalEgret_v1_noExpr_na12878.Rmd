...
Running finalEgret v1 (*without Expr data*) on GM12878.
...
tag <- "finalEGRET_v1_noExpr_na12878_06192020"
```
...
mclapply(processes, function(p) {
  system.time(results <- panda(as.data.frame(prior_list[p]), expr=clean_data[,c(1,2)], ppi=ppiFiltered,progress=TRUE, remove.missing.ppi = TRUE, remove.missing.motif = TRUE, remove.missing.genes = TRUE))
