...
bbseq <- read.table("../ase/ase_mapped_09012020.txt", header = FALSE)
colnames(bbseq) <- c("tf","gene","pval","fdr")
bbseq$id <- paste0(bbseq$tf,bbseq$gene)
bbseq_signif <- bbseq[which(bbseq$fdr <= 0.1),]
bbseq_signif$id <- paste0(bbseq_signif$tf,bbseq_signif$gene)
...
combined_table_bbseq_tested_edge <- combined_table[which(combined_table$id %in% bbseq$id),]
combined_table$bbseq_gene_promotor <- ifelse(combined_table$gene %in% bbseq_promotor_signif$gene, 1, 0)
combined_table_bbseq_tested_gene_promotor <- combined_table[which(combined_table$id %in% bbseq_promotor$id),]
...
test if top difference edges between genotypes are enriched in diff chipseq
```{r}
data <- combined_table
...
do ASE and nfkb enrichment on whole combined data, not just that whcih overlaps with chipseq
```{r}
...
combined_table_all$diff_diff_egret_k_g <- abs(combined_table_all$diff_k-combined_table_all$diff_g)
combined_table_all$diff_egret_egret_k_g <- abs(combined_table_all$egret_k562-combined_table_all$egret_gm12878)
#combined_table_all$edited_k <- ifelse(combined_table_all$id %in% mods_k562$id, 1, 0)
#combined_table_all$edited_g <- ifelse(combined_table_all$id %in% mods_gm12878$id, 1, 0)
#combined_table_all$eqtl <- ifelse(combined_table_all$id %in% eqtls$id, 1, 0)
#mark significant bbseq edges
#combined_table_all$bbseq_edge <- ifelse(combined_table_all$id %in% bbseq_signif$id, 1, 0)
#combined_table_all$bbseq_gene <- ifelse(combined_table_all$gene %in% bbseq_signif$gene, 1, 0)
#narrow down to only those edges which were "testable"
#combined_table_bbseq_tested <- combined_table_all[which(combined_table_all$id %in% bbseq$id),]
...
gene_ag <- aggregate(combined_table_all_bbseq_tested_gene_promotor$diff_diff_egret_k_g, by = list(combined_table_all_bbseq_tested_gene_promotor$gene), FUN = sum)
...
mylist <- as.vector(combined_table_all_caqtl_tested[order(-as.numeric(as.vector(combined_table_all_caqtl_tested$diff_diff_egret_k_g))),1])
