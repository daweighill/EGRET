---
title: "Final EGRET v1 validation: GM12878 (level3Neg all QBiC models)"
...
library(precrec)
library(ggplot2)
tag <- "finalEgret_v1_GM12878_allModels_level3Neg_06292020"
...
files <- c("finalEgret_v1_na12878_allModels_level3Neg_06292020_egret.RData")
...
for (i in c(4:5)){
  ...
  curves <- evalmod(scores = filtered_net_table[,i], labels = as.factor(filtered_net_table$chipseq),modnames = c(specs))
  autoplot(curve)
  aucs <- auc(curves)
  print(aucs)
}
...
egret_priors <- c(1)
thresholds <- c(0.1, 0.15, 0.2, 0.25, 0.5, 0.75,1)
