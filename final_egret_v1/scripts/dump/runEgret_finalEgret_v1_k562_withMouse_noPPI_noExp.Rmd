---
title: "Run finalEgret v1 - K562 with mouse, noPPI, noExp"
...
Running finalEgret v1 (*with all data types*) on K562, including mouse predictions from qbic.
...
tag <- "finalEgret_v1_k562_withMouse_noPPI_noExp_06292020"
```
...
qbic <- read.table(file = "../inputs/qbic_eQTLS_GM12878_K562_finalEGRET_v1_06182020_withMouse.txt", header = FALSE)

  
# individual genotype
vcf <- read.table("../genotype_data/K562_formatted.vcf", header = FALSE, sep = "\t")
```
...
```{r parse_genotype}
colnames(vcf) <- c("CHROM",  "POS"  ,   "ID"  ,    "REF"  ,   "ALT"   ,  "QUAL"   , "FILTER" , "INFO"   , "FORMAT", "GENOTYPE","alt_allele_count")
snp_ids <- paste0(vcf$CHROM,"_",vcf$POS)
rownames(vcf) <- snp_ids
vcf$snp_id <- snp_ids

#assign alt allele count of the individual to the qbic table
qbic_ag$alt_allele_count <- as.numeric(as.vector(vcf$alt_allele_count))[match(qbic_ag$snpID, vcf$snp_id)]

#assign alt allele count of the individual to the QTL table
qtl$alt_allele_count <- as.numeric(as.vector(vcf$alt_allele_count))[match(qtl$snpID, vcf$snp_id)]
...
mclapply(processes, function(p) {
  system.time(results <- panda(as.data.frame(prior_list[p]), expr=clean_data[,c(1,2)], ppi=NULL,progress=TRUE, remove.missing.ppi = TRUE, remove.missing.motif = TRUE, remove.missing.genes = TRUE))
