...
library(plyr)
# GM12878
load("../outputs/validation_table_finalEgret_v1_GM12878_allModels_level2Bi_06292020.RData")
table_gm12878 <- validation_table
colnames(table_gm12878) <- c("id","tf","gene","panda","egret_gm12878","chipseq_gm12878","diff_gm12878")

# K562
load("../outputs/validation_table_finalEgret_v1_K562_allModels_level2Bi_06292020.RData")
...
```{r}
combined_table[which((combined_table$chipseq_k562 != combined_table$chipseq_gm12878) & (combined_table$diff_gm12878 > 0.5)),c(1:11)]
```
...
library(gridExtra)
library(pROC)
top_x_table_gm12878 <- table_gm12878[which(table_gm12878$diff_gm12878 > 0.5),][,c(4,5,6)]
...
pdf("gm12878_top35_curves_lenient.pdf",width = 9, height = 3)
plot(p3)
dev.off()
```
...
library(gridExtra)
library(ggplot2)
top_x_table_k562 <- table_k562[which(table_k562$diff_k562 > 0.5),][,c(4,5,6)]
...
p3 <- grid.arrange(arrangeGrob(roccurves + theme(legend.position="none"), prcurves + theme(legend.position="none"), nrow=1, widths = c(4,4)), mylegend, nrow=1, widths = c(8,2), newpage = FALSE)
pdf("k562_top35_curves_lenient.pdf",width = 9, height = 3)
plot(p3)
dev.off()
```
