...
#
# output file
my $level2Neg = "/home/ubuntu/EGRET/final_egret_v1/inputs/qbic_allSNPS_GM12878_K562_finalEGRET_v1_06182020.txt";
...
while (<FILE>)
{
	if ($_ !~ /wild/)
	{
		...
		for my $t (@tfs)
			...
			if (($refstatus ne $altstatus) and ($diff < 0) and ($line[8] =~ /Homo_sapiens/))
			{
