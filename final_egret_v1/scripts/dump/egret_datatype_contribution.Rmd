...
Do another one with E - |AB|
```{r}
pallet <- c("#FFBF00",  "#870E75"  ,  "#0BB19F" )

p <- ggplot(data = egret_auc_pvals_gm12878, aes(x = reorder(name,-as.numeric(improvement)), y = improvement)) + geom_bar(stat="identity", fill = pallet[3]) + theme_bw()  +  theme(text = element_text(size=12))  +  geom_text(aes(label=signif(improvement,3)))+  scale_x_discrete("EGRET prior modification",labels=c("E - |qAB|","E - |AB|","E - |B|","E - |qA|")) +  labs(y = "% improvement in AUC-ROC", title = "Contribution of EGRET data types")
#plot(p)
