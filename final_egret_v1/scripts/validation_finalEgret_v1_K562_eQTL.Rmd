---
title: "Final Egret v1 validation: K562"
...
library(precrec)
library(ggplot2)
tag <- "finalEgret_v1_K562_06182020"
chipseq <- "../../ca_egret/ca_egret_v3.1_LOO/validation/K562_remap2018_all_macs2_hg19_v1_2.bed"
```
...
filtered_net_table$diff_egret <- abs(filtered_net_table$egret_k562 - filtered_net_table$panda)
...
get_roc_for_diff_thresh <- function(egretNum, thresh) {
  ...
  testB <- roc.test(rocP, rocE, alternative = "less", method = "bootstrap")
  pandaAUC <- testD$estimate["AUC of roc1"]
  egretAUC <- testD$estimate["AUC of roc2"]
  pvalD <- testD$p.value
  pvalB <- testB$p.value
  print(paste(egretNum,thresh,pvalD,pvalB))
  ...
  result <- c(egretNum, names[egretNum], thresh, nn, np, pandaAUC, egretAUC, pvalD,pvalB)
  return(result)
  autoplot(curves)
}

egret_priors <- c(1)
thresholds <- c(0.1, 0.15, 0.2, 0.25, 0.5, 0.75,1,2,3,4)
...
colnames(egret_auc_pvals) <- c("egretNum", "name", "thresh", "nn", "np", "pandaAUC", "egretAUC", "pvalD","pvalB")
write.table(egret_auc_pvals, file = filename, sep = "\t", col.names = TRUE, quote=FALSE)
egret_auc_pvals
```

# validate "top x" different edges
...
```{r}
validation_table$diff_egret
validation_table_thresh <- validation_table[which(validation_table$diff_egret > 0.15),]
ggplot(validation_table_thresh, aes(x = validation_table_thresh$diff_egret)) + geom_histogram(binwidth = 2)
```
...
egret_priors <- c(1)
#thresholds <- c(0.5)
thresholds <- c(0.05,0.1,0.25,0.5,0.75,1,2,3)
...
```{r}
curve <- evalmod(scores = -validation_table_thresh$egret_k562, labels = validation_table_thresh$disrupt, posclass = 1)
autoplot(curve)
aucs <- auc(curve)
aucs
```