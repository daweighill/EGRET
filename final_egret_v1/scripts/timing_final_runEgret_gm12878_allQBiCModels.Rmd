...
tag <- "timing_EGRET_allQBiCModels_gm12878_08162021"

write("runEGRET script start", file="runEGRET_timing.txt")
write(capture.output(Sys.time()), file="runEGRET_timing.txt", append=TRUE)
```

# Inputs
```{r}
write("read inputs and make prior", file="runEGRET_timing.txt", append=TRUE)
write(capture.output(Sys.time()), file="runEGRET_timing.txt", append=TRUE)
```
Individual inputs:
```{r individual_inputs}
...
```{r}
write("run message passing", file="runEGRET_timing.txt", append=TRUE)
write(capture.output(Sys.time()), file="runEGRET_timing.txt", append=TRUE)
```
...
# Run PANDA Message Passing 
```{r}
write("end", file="runEGRET_timing.txt", append=TRUE)
write(capture.output(Sys.time()), file="runEGRET_timing.txt", append=TRUE)
```
