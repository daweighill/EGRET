...
names <- c("noExp_noPPI","panda_noExp_noPPI","onlyQTL","onlyQbic","allDataTypes")
...
names <- c("onlyQTL","onlyQbic","allDataTypes")
diff_cols <- c(12,13,14)
egret_net_nums <- c(7,8,9)
...
labels=c("all mods","only QTL mods","only QBiC mods","no PPI/Expr")
...
p <- ggplot(data = egret_auc_pvals_gm12878, aes(x = reorder(name,-as.numeric(improvement)), y = improvement)) + geom_bar(stat="identity", fill = pallet[3]) + theme_bw()  +  theme(text = element_text(size=12))  +  geom_text(aes(label=signif(improvement,3)))+  scale_x_discrete("EGRET prior modifications",labels=c("all mods","only QTL mods","only QBiC mods")) +  labs(y = "Perc. improvement in AUC-ROC", title = "Contribution of EGRET data types")

pdf("../figures/egret_data_type_contribution.pdf",width = 5, height = 4)
plot(p)
dev.off()
```
