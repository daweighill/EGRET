...
save(tf_degrees, file = "../outputs/tf_diff_degree_table_banovich_smart1_07042020.RData")
save(gene_degrees, file = "../outputs/gene_diff_degree_table_banovich_smart1_7042020.RData")
...
BMI_enrich <- sapply(individuals, FUN = function(i){
  ...
  high_diff_gwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_BMI_tf == 1)))
  low_diff_gwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_BMI_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_BMI_tf == 0)))
  low_diff_notGwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_BMI_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
...
ggplot(cd_df) + geom_point(shape = 19, aes(x = reorder(Individual, log10(pvalue)), y = -log10(pvalue), col = Tissue),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```
...
CD_enrich <- sapply(individuals, FUN = function(i){
  top_diff_tfs <- top_n(tf_degrees, top,tf_degrees[,i])
  high_diff_gwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_CD_tf == 1)))
  low_diff_gwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_CD_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_CD_tf == 0)))
  low_diff_notGwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_CD_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(CD_enrich) <- colnames(tf_degrees)[individuals]
cd_enrich_individuals <- names(which(CD_enrich < 0.05))
cd_enrich_individuals
...
ggplot(cad_df) + geom_point(shape = 19, aes(x = reorder(Individual, log10(pvalue)), y = -log10(pvalue), col = Tissue),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```
...
names(CAD_enrich) <- colnames(tf_degrees)[individuals]
cad_enrich_individuals <- names(which(CAD_enrich < 0.05))
cad_enrich_individuals
...
MI_enrich <- sapply(individuals, FUN = function(i){
  high_diff_gwas <- length(which((tf_degrees_scaled[,i] >d) & (tf_degrees_scaled$GWAS_MI_tf == 1)))
  low_diff_gwas <- length(which((tf_degrees_scaled[,i] <=d) & (tf_degrees_scaled$GWAS_MI_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees_scaled[,i] >d) & (tf_degrees_scaled$GWAS_MI_tf == 0)))
  low_diff_notGwas <- length(which((tf_degrees_scaled[,i] <=d) & (tf_degrees_scaled$GWAS_MI_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(MI_enrich) <- colnames(tf_degrees_scaled)[individuals]

MI_enriched_indiv <- names(which(MI_enrich < 0.05))
MI_enriched_indiv
```
...
MI_enrich <- sapply(individuals, FUN = function(i){
  top_diff_tfs <- top_n(tf_degrees, top,tf_degrees[,i])
  high_diff_gwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_MI_tf == 1)))
  low_diff_gwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_MI_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_MI_tf == 0)))
  low_diff_notGwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_MI_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(MI_enrich) <- colnames(tf_degrees)[individuals]
mi_enrich_individuals <- names(which(MI_enrich < 0.05))
mi_enrich_individuals
```
...
RA_enrich <- sapply(individuals, FUN = function(i){
  high_diff_gwas <- length(which((tf_degrees_scaled[,i] >d) & (tf_degrees_scaled$GWAS_RA_tf == 1)))
  low_diff_gwas <- length(which((tf_degrees_scaled[,i] <=d) & (tf_degrees_scaled$GWAS_RA_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees_scaled[,i] >d) & (tf_degrees_scaled$GWAS_RA_tf == 0)))
  low_diff_notGwas <- length(which((tf_degrees_scaled[,i] <=d) & (tf_degrees_scaled$GWAS_RA_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(RA_enrich) <- colnames(tf_degrees_scaled)[individuals]
...
d = 2
individuals <- c(c(3:121),c(123:241),c(243:361))
MS_enrich <- sapply(individuals, FUN = function(i){
  high_diff_gwas <- length(which((tf_degrees_scaled[,i] >d) & (tf_degrees_scaled$GWAS_MS_tf == 1)))
  low_diff_gwas <- length(which((tf_degrees_scaled[,i] <=d) & (tf_degrees_scaled$GWAS_MS_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees_scaled[,i] >d) & (tf_degrees_scaled$GWAS_MS_tf == 0)))
  low_diff_notGwas <- length(which((tf_degrees_scaled[,i] <=d) & (tf_degrees_scaled$GWAS_MS_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(MS_enrich) <- colnames(tf_degrees_scaled)[individuals]
ms_enriched_indiv <- names(which(MS_enrich < 0.05))
ms_enriched_indiv
```
...
RA_enrich <- sapply(individuals, FUN = function(i){
  top_diff_tfs <- top_n(tf_degrees, top,tf_degrees[,i])
  high_diff_gwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_RA_tf == 1)))
  low_diff_gwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_RA_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees$GWAS_RA_tf == 0)))
  low_diff_notGwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees$GWAS_RA_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(RA_enrich) <- colnames(tf_degrees)[individuals]
ra_enrich_individuals <- names(which(RA_enrich < 0.05))
ra_enrich_individuals
```
...
MS_enrich <- sapply(individuals, FUN = function(i){
  top_diff_tfs <- top_n(tf_degrees, top,tf_degrees[,i])
  high_diff_gwas <- length(which((tf_degrees_scaled$tf %in%  top_diff_tfs$tf) & (tf_degrees_scaled$GWAS_MS_tf == 1)))
  low_diff_gwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees_scaled$GWAS_MS_tf == 1)))
  high_diff_notGwas <- length(which((tf_degrees$tf %in%  top_diff_tfs$tf) & (tf_degrees_scaled$GWAS_MS_tf == 0)))
  low_diff_notGwas <- length(which((!(tf_degrees$tf %in%  top_diff_tfs$tf)) & (tf_degrees_scaled$GWAS_MS_tf == 0)))
  contingency_table <- matrix(c(high_diff_gwas,low_diff_gwas,high_diff_notGwas,low_diff_notGwas), nrow = 2,dimnames = list(Diff = c("High", "Low"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(MS_enrich) <- colnames(tf_degrees)[individuals]
ms_enrich_individuals <- names(which(MS_enrich < 0.05))
ms_enrich_individuals
```
...
cm_trait <- length(which(ipsc_cm_indiv %in% cad_enriched_indiv))
Ncm_trait <- length(which((lcl_indiv %in% cad_enriched_indiv))) + length(which((ipsc_indiv %in% cad_enriched_indiv)))
cm_Ntrait <- length(which(!((ipsc_cm_indiv %in% cad_enriched_indiv))))
Ncm_Ntrait <- length(which(!((lcl_indiv %in% cad_enriched_indiv)))) + length(which(!((ipsc_indiv %in% cad_enriched_indiv))))
...
p <- ggboxplot(tf_degree_iPSC_CM, outlier.shape = NULL, x = "CAD", y = "diff_out_degree", color = "CAD", palette = c("royalblue3", "firebrick2")) + stat_compare_means(method = "t.test", method.args = list(alternative = "greater"))  + theme(legend.position="right") #+ labs(title="Top 35 disruption scores - GM12878", x ="ChIP-seq binding", y 
#= "Edge Disruption Score") 
plot(p)
#pdf("gm12878_top35_diffvalues_boxplot.pdf",width = 5, height = 3.5)
#plot(p)
#dev.off()
t.test(tf_degree_iPSC_CM[which(tf_degree_iPSC_CM$CAD == 1),]$diff_out_degree,tf_degree_iPSC_CM[which(tf_degree_iPSC_CM$CAD == 0),]$diff_out_degree, alternative="greater")

colnames(tf_degree_iPSC_CM) <-  c("tf","individual","diff_out_degree","tissue")
tf_degree_iPSC_CM$CD <- tf_degrees_scaled$GWAS_CD_tf[match(tf_degree_iPSC_CM$tf, tf_degrees_scaled$tf)]
p <- ggboxplot(tf_degree_iPSC_CM, outlier.shape = NULL, x = "CD", y = "diff_out_degree", color = "CD", palette = c("royalblue3", "firebrick2")) + stat_compare_means(method = "t.test", method.args = list(alternative = "greater"))  + theme(legend.position="right") #+ labs(title="Top 35 disruption scores - GM12878", x ="ChIP-seq binding", y 
#= "Edge Disruption Score") 
plot(p)
#pdf("gm12878_top35_diffvalues_boxplot.pdf",width = 5, height = 3.5)
#plot(p)
...
p <- ggboxplot(tf_degree_lcl, outlier.shape = NULL, x = "CD", y = "diff_out_degree", color = "CD", palette = c("royalblue3", "firebrick2")) + stat_compare_means(method = "t.test", method.args = list(alternative = "greater"))  + theme(legend.position="right") #+ labs(title="Top 35 disruption scores - GM12878", x ="ChIP-seq binding", y 
#= "Edge Disruption Score") 
plot(p)
#pdf("gm12878_top35_diffvalues_boxplot.pdf",width = 5, height = 3.5)
#plot(p)
...
p <- ggboxplot(tf_degree_lcl, outlier.shape = NULL, x = "CAD", y = "diff_out_degree", color = "CAD", palette = c("royalblue3", "firebrick2")) + stat_compare_means(method = "t.test", method.args = list(alternative = "greater"))  + theme(legend.position="right") #+ labs(title="Top 35 disruption scores - GM12878", x ="ChIP-seq binding", y 
#= "Edge Disruption Score") 
plot(p)
#pdf("gm12878_top35_diffvalues_boxplot.pdf",width = 5, height = 3.5)
#plot(p)
...
Test to see if tissue-specific disruptions are associated with GWAS genes of certain disease

## iPSC-CM specific disrupted tfs
```{r}
top = 100
#load("../outputs/tf_diff_degree_table.RData")
individuals <- c(1:119)
lcl_indiv <- c(3:121)
iPSC_indiv <- c(123:241)
iPSC_CM_indiv <- c(243:361)
iPSC_CM_enrich_CAD <- sapply(individuals, FUN = function(i){
  #get list of tfs which are "top n" for that individual for each cell type
  top_diff_tfs_ipsc_cm <- top_n(tf_degrees, top,tf_degrees[,iPSC_CM_indiv[i]])$tf
  top_diff_tfs_ipsc <- top_n(tf_degrees, top,tf_degrees[,iPSC_indiv[i]])$tf
  top_diff_tfs_lcl <- top_n(tf_degrees, top,tf_degrees[,lcl_indiv[i]])$tf
  
  ipsc_cm_specific <- which(!(top_diff_tfs_ipsc_cm %in% top_diff_tfs_lcl))
lcl_specific <- which(!(top_diff_tfs_lcl %in% top_diff_tfs_ipsc_cm))
  
  # make contingency table 
  # specific, not specific, gwas, not gwas
  specific_gwas <- length(which(tf_degrees$tf %in% ipsc_cm_specific) & (tf_degrees$GWAS_CAD_tf == 1))
  nonspecific_gwas <- length(which((!(tf_degrees$tf %in% ipsc_cm_specific)) & (tf_degrees$GWAS_CAD_tf == 1)))
  specific_nongwas <- length(which(tf_degrees$tf %in% ipsc_cm_specific) & (tf_degrees$GWAS_CAD_tf == 0))
  nonspecific_nongwas <- length(which((!(tf_degrees$tf %in% ipsc_cm_specific)) & (!(tf_degrees$GWAS_CAD_tf == 1))))
  contingency_table <- matrix(c(specific_gwas,nonspecific_gwas,specific_nongwas,nonspecific_nongwas), nrow = 2,dimnames = list(Diff = c("Specific", "Non-specific"),GWAS = c("yes", "no")))
  test <- fisher.test(contingency_table, alternative = "greater")
  return(as.numeric(test$p.value))
})
names(iPSC_CM_enrich_CAD) <- colnames(tf_degrees)[individuals]
ipsc_cm_cad_enrich_individuals <- names(which(iPSC_CM_enrich_CAD < 0.05))
ipsc_cm_cad_enrich_individuals
```
...
save(t_test_genes, file = "../outputs/t_test_gene_degree_diff_banovich_smart1_07042020.RData")

```


# Do a  t-test - tfs
Which TFs have significantly different disruption scores across different tissues
```{r}
...
for (i in 1:(dim(t_test_tfs)[1])){
  x <- tf_degrees[i,]
  #lcl_vs_ipsc <- t.test(as.numeric(as.vector(x[c(3:121)])), as.numeric(as.vector(x[c(123:241)])), alternative = "two.sided")
  ipsc_vs_ipscCM <- t.test(as.numeric(as.vector(x[c(123:241)])), as.numeric(as.vector(x[c(243:361)])), alternative = "less")
  lcl_vs_ipscCM <- t.test(as.numeric(as.vector(x[c(3:121)])), as.numeric(as.vector(x[c(243:361)])), alternative = "less")
...
#save(t_test_tfs, file = "../outputs/t_test_TFs_degree_diff_banovich_smart1_07042020.RData")
#load("../outputs/")
...
tf_ttest_comparison_cad <- melt(t_test_tfs[which(t_test_tfs$GWAS_CAD_tf == 1),c(1,11,12)])
colnames(tf_ttest_comparison_cad) <- c("tf","Comparison","FDR")

ggplot(tf_ttest_comparison_cad) + geom_point(shape = 19, aes(x = reorder(tf, log10(FDR)), y = -log10(FDR), col = Comparison),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```


```{r}
t_test_tfs[which(t_test_tfs$GWAS_RA_tf == 1),]
tf_ttest_comparison_ra <- melt(t_test_tfs[which(t_test_tfs$GWAS_RA_tf == 1),c(1,11,12,13)])
colnames(tf_ttest_comparison_ra) <- c("tf","Comparison","FDR")

ggplot(tf_ttest_comparison_ra) + geom_point(shape = 19, aes(x = reorder(tf, log10(FDR)), y = -log10(FDR), col = Comparison),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```


```{r}
t_test_tfs[which(t_test_tfs$GWAS_CD_tf == 1),]
tf_ttest_comparison_cd <- melt(t_test_tfs[which(t_test_tfs$GWAS_CD_tf == 1),c(1,11,12,13)])
colnames(tf_ttest_comparison_cd) <- c("tf","Comparison","FDR")

ggplot(tf_ttest_comparison_cd) + geom_point(shape = 19, aes(x = reorder(tf, log10(FDR)), y = -log10(FDR), col = Comparison),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```
...
```{r}
t_test_tfs[which(t_test_tfs$GWAS_BMI_tf == 1),]
tf_ttest_comparison_bmi <- melt(t_test_tfs[which(t_test_tfs$GWAS_BMI_tf == 1),c(1,11,12,13)])
colnames(tf_ttest_comparison_bmi) <- c("tf","Comparison","FDR")

ggplot(tf_ttest_comparison_bmi) + geom_point(shape = 19, aes(x = reorder(tf, log10(FDR)), y = -log10(FDR), col = Comparison),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```


```{r}
t_test_tfs[which(t_test_tfs$GWAS_MI_tf == 1),]
tf_ttest_comparison_mi <- melt(t_test_tfs[which(t_test_tfs$GWAS_MI_tf == 1),c(1,11,12,13)])
colnames(tf_ttest_comparison_mi) <- c("tf","Comparison","FDR")

ggplot(tf_ttest_comparison_mi) + geom_point(shape = 19, aes(x = reorder(tf, log10(FDR)), y = -log10(FDR), col = Comparison),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```

```{r}
t_test_tfs[which(t_test_tfs$GWAS_MS_tf == 1),]
tf_ttest_comparison_ms <- melt(t_test_tfs[which(t_test_tfs$GWAS_MS_tf == 1),c(1,11,12,13)])
colnames(tf_ttest_comparison_ms) <- c("tf","Comparison","FDR")

ggplot(tf_ttest_comparison_ms) + geom_point(shape = 19, aes(x = reorder(tf, log10(FDR)), y = -log10(FDR), col = Comparison),) + theme_classic()+ scale_color_manual(values=c("red", "blue","orange","cyan", "darkgreen", "violet")) + geom_abline(slope = 0, intercept = -log10(0.05), linetype = "dashed", col = "black") + xlab("Individual") + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())
```
