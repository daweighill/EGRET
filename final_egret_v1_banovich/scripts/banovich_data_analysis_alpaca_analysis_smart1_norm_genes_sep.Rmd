...
```{r}
high_score_nodes_cad <- unique(cads_melted[which(((cads_melted$alpaca_score) > 2) & (cads_melted$cellType == "iPSCCM")),'node'])
...
disruptions <- melt(gene_degrees[which(gene_degrees$gene == "ENSG00000070366"),c(1,c(3:121),c(123:241),c(243:361))])
...
cds <- merged_alpaca_table_scaled[which(merged_alpaca_table_scaled$node %in% CD$gene),c(1:358)]
cds_melted <- melt(cds)
...
cds <- merged_alpaca_table_scaled[which(merged_alpaca_table_scaled$node %in% high_score_nodes_cd),c(1:358)]
cds_melted <- melt(cds)
colnames(cds_melted) <- c("node","sample","alpaca_score")
cds_melted <- separate(cds_melted, sample, c("cellType","tag", "indiv"), "_", remove = TRUE)
...
pdf("banovich_topCD_lcl.pdf",width = 5, height = 3.5)
plot(p2)
dev.off()
plot(p2)
