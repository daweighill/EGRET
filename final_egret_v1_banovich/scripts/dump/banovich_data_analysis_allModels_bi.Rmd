...
# Overview
Here we do some analysis on the banovich results for the "application". This uses all QBiC models with bi, lenient thresholding.
...
library(reshape2)
# panda"
load("../outputs/finalEgret_v1_banovich_LCL_allModels_06302020_1_panda.RData")
net <- melt(regnet)
...
for (g in c(1:119)) {
  filename <- paste0("../outputs/finalEgret_v1_banovich_LCL_allModels_06302020_",g,"_egret.RData")
  load(filename)
  net <- melt(regnet)
  colnames(net) = c("tf","gene","score")
  net$id <- paste0(net$tf,net$gene)
...
save(regnet_edge_table_LCL, file = "../outputs/LCL_table_banovich_finalEgret_v1_allModels_07012020.RData")
```


## Load the iPSC results
```{r echo=FALSE,results='hide'}
load("../outputs/finalEgret_v1_banovich_iPSC_allModels_06302020_1_panda.RData")
net <- melt(regnet)
colnames(net) = c("tf","gene","score")
net$id <- paste0(net$tf,net$gene)
...
for (g in c(1:119)) {
  filename <- paste0("../outputs/finalEgret_v1_banovich_iPSC_allModels_06302020_",g,"_egret.RData")
  load(filename)
  net <- melt(regnet)
  colnames(net) = c("tf","gene","score")
  net$id <- paste0(net$tf,net$gene)
...
save(regnet_edge_table_iPSC, file = "../outputs/iPSC_table_banovich_finalEgret_v1_allModels_07012020.RData")
```

## Load the iPSC-CM results
```{r echo=FALSE,results='hide'}
load("../outputs/finalEgret_v1_banovich_iPSC-CM_allModels_06302020_1_panda.RData")
net <- melt(regnet)
colnames(net) = c("tf","gene","score")
net$id <- paste0(net$tf,net$gene)
...
for (g in c(1:119)) {
  filename <- paste0("../outputs/finalEgret_v1_banovich_iPSC-CM_allModels_06302020_",g,"_egret.RData")
  load(filename)
  net <- melt(regnet)
  colnames(net) = c("tf","gene","score")
  net$id <- paste0(net$tf,net$gene)
...
save(regnet_edge_table_iPSC_CM, file = "../outputs/iPSC-CM_table_banovich_finalEgret_v1_allModels_07012020.RData")
```
...
save(merged_regnet_table, file = "../outputs/merged_regnet_table_banovich_allModels_07012020.RData")
```
...
save(diff_table, file = "../outputs/diffTable_merged_LCL_iPSC_iPSC-CM_banovich_edge_table_allModels_07012020.RData")
```
...
# combine
diff_melted_combined <- rbind(diff_melted_LCL,diff_melted_iPSC, diff_melted_iPSC _CM)
...
save(diff_table, file = "../outputs/diff_table_with_gwas_banovich_allModels_07012020.RData")
```
