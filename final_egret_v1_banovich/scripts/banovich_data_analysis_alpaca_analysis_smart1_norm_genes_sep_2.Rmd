...
plot distributions
```{r}
alpaca_table <- merged_alpaca_table_scaled2
...
alpaca_table <- merged_alpaca_table_scaled_qq
alpaca_lcl <- melt(alpaca_table[,c(1,c(2:120))], id.vars = c("node"))
alpaca_lcl$tissue <- "LCL"
alpaca_iPSC <- melt(alpaca_table[,c(1,c(121:239))], id.vars = c("node"))
alpaca_iPSC$tissue <- "iPSC"
...
```{r}
data <- merged_alpaca_table_scaled_qq
...
```{r}
data <- merged_alpaca_table_scaled_qq
high_score_nodes_cd <- unique(cds_melted[which((abs(cds_melted$alpaca_score) > 2) & (cds_melted$cellType == "LCL")),'node'])
...
p1 <- ggplot(genes_melted[which(genes_melted$cellType == "iPSCCM"),]) + geom_point(size = 2,alpha = 0.4,aes(x = reorder(indiv, -alpaca_score), y = (alpaca_score), col = cellType,  shape = node)) + theme_classic()+ scale_color_manual(values=pallet)  + theme( axis.text.x=element_blank(), axis.ticks.x=element_blank())+ labs(y = "ALPACA score",col = "Cell type",shape = "TF", x= "Individual") 

p2 <- annotate_figure(p1,top = text_grob("ALPACA scores for example gene", color = "black", face = "bold", size = 14))
pdf("banovich_topCAD_ipsc.pdf",width = 5, height = 3.5)
plot(p2)
dev.off()
